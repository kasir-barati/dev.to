...
- `series`: derived from the article's directory, not hand-typed. An article directly in `articles/` must have no `series` key; putting it one level down in `articles/<series-slug>/<article-slug>.md` (kebab-case dir, e.g. `pipeline-pattern`) gets `series: Pipeline Pattern` auto-written by `scripts/apply_series_from_dir.py`. Reserved directory names (`TIL`, `assets`, `DRAFT`, `JA`, or any other gitignored dir) are never treated as a series.
...
- `articles/*.md` (flat) and `articles/<series-slug>/*.md` (exactly one level deep, series articles): published articles. Only these are synced to dev.to — the workflows use a `:(glob)`/`:(exclude,glob)` git pathspec set (see `scripts/series_dirs.py`'s `RESERVED_DIRS`) to include both shapes while excluding `TIL/`, `assets/`, `DRAFT/`, `JA/`. Anything nested more than one level deep is a validation error.
- `articles/TIL/`, `articles/assets/<slug>/`: TIL notes and per-article assets (images, `diagrams/*.d2` sources + rendered PNGs). Assets always live at `articles/assets/<slug>/`, never under a series directory, regardless of where the article itself lives. Reference them from a flat article with `./assets/<slug>/...`, or from a series article (one directory deeper) with `../assets/<slug>/...` — devto-cli resolves the relative path against the article's own location before rewriting it (`path.normalize(path.join(dirname, imagePath))`, verified against `@sinedied/devto-cli@1.4.0`'s `lib/util.js`), so it must match the article's actual depth. Both forms resolve to absolute `raw.githubusercontent.com` URLs after `dev push` rewrites them — but only inside markdown image syntax (`![]()`); a plain `[]()` link or an `<img src>` pointing at an asset is never rewritten and must use an absolute `raw.githubusercontent.com` URL by hand instead. A `?v=<token>` query string sometimes appended to an image reference is written automatically by `scripts/bump_asset_versions.py` when its asset changes — never hand-author or hand-edit one.
- `articles/DRAFT/`, `articles/JA/`: gitignored, local-only (drafts, Japanese translations. You can create new gitignored dirs for other languages the same way — `scripts/series_dirs.py`'s `discover_paths()`/`classify()` treat any gitignored directory as reserved automatically, no code change needed). Never reach dev.to or git history.
...
- `make setup` also runs `pre-commit install`, registering a git hook (`.pre-commit-config.yaml`) that runs `make validate` (full-corpus `validate_articles.py --all`) before each commit — the same check `validate.yml` gates on, just earlier and over the whole repo rather than only staged files. It doesn't catch an asset-only edit or a reference to an unstaged asset (CI still catches both).
